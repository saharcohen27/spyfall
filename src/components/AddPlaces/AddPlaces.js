...
function AddedPlace({addedPlace, index, handleRemove}){
  return (
    <div className="added-place">
      <div className="place-text">{addedPlace.length > 8 ? `${addedPlace.slice(0,8)}...` : addedPlace}</div>
      <RemoveIcon className="change-btn delete" onClick={() => handleRemove(index)}/>
    </div>
  )
}
        ...
        <div className="added-places">
          {
            addedPlaces.map((addedPlace, index) => (
              <AddedPlace addedPlace={addedPlace} index={index} handleRemove={handleRemove} /> 
            ))
          }
        </div>
      }
